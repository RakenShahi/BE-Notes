...
public class Questions_RecyclerViewActivity extends AppCompatActivity {
    ;
                    ...
                    .addValueEventListener(new ValueEventListener() {
                        ...
                        public void onDataChange(DataSnapshot dataSnapshota) {
                            ...
                            if (university.isEmpty() || faculty.isEmpty() || semester.isEmpty()) {
                                Toast.makeText(Questions_RecyclerViewActivity.this, "Fields value error", Toast.LENGTH_SHORT).show();
                            } else {
//                                Toast.makeText(Syllabus_RecyclerViewActivity.this, university + " " + faculty + " " + semester, Toast.LENGTH_SHORT).show();

            DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference().child("Questions").child(university).child(faculty).child(semester);
            databaseReference.addChildEventListener(new ChildEventListener() {
                ...
                public void onChildAdded(DataSnapshot dataSnapshotb, String s) {
                    ...
                    DatabaseReference bhitra=FirebaseDatabase.getInstance().getReference().child("Questions").child(university).child(faculty).child(semester).child(key);
                        bhitra.addValueEventListener(new ValueEventListener() {
                            @Override
                            public void onDataChange(DataSnapshot dataSnapshot) {
                              fileName=String.valueOf(dataSnapshot.child("subject").getValue());
                              String year=String.valueOf(dataSnapshot.child("Q_year").getValue());
                              String newword= fileName + " "+ year;
                              url = String.valueOf(dataSnapshot.child("Url").getValue());
                                ((QuestionsAdapter) recyclerView.getAdapter()).update(newword, url);
...
//                String url=dataSnapshot.`getValue(String.class); //return url for filename
        ...
        recyclerView.setLayoutManager(new LinearLayoutManager(Questions_RecyclerViewActivity.this));
        QuestionsAdapter myAdapter=new QuestionsAdapter(recyclerView,Questions_RecyclerViewActivity.this,new ArrayList<String>(), new ArrayList<String>());
    ...
    private void Logout(){
        ...
        finish();
        Intent intent= new Intent(Questions_RecyclerViewActivity.this,FirebaseLogin.class);
