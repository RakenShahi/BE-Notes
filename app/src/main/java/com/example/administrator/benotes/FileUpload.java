...
public class FileUpload extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        author=(EditText)findViewById(R.id.author_txt);
        ...
        upload.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                {   sub=subject.getText().toString().trim();
                    note_author=author.getText().toString().trim();
                    ...
                    if (sub.isEmpty() || note_author.isEmpty() || university.isEmpty() || semestervalue.isEmpty()||facultyvalue.isEmpty()) {

                        Toast.makeText(FileUpload.this, "Subject name and Author name required", Toast.LENGTH_SHORT).show();
    ...
    private void uploadFile(Uri pdfUri) {
        ...
        StorageReference storageReference = storage.getReference(); // returns root path
            ...
            storageReference.child("Notes").child(university).child(facultyvalue).child(semestervalue).child(sub).child(filename).putFile(pdfUri)
...
//                            Toast.makeText(FileUpload.this, url, Toast.LENGTH_SHORT).show();
                            ...
                            NotesInfo notesInfo = new NotesInfo(filename1,sub,note_author,url,filename);
                            reference.child("Notes").child(university).child(facultyvalue).child(semestervalue).child(filename1).setValue(notesInfo).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    progressDialog.dismiss();
                                    if (task.isSuccessful()) {
                                        Toast.makeText(FileUpload.this, "File is successfuly uploaded", Toast.LENGTH_SHORT).show();
                                    } else {
                                        Toast.makeText(FileUpload.this, "File realtime ma upload vayena", Toast.LENGTH_SHORT).show();
                                    }
                                }
                ...
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(FileUpload.this, "File is not successfuly uploaded..vayena", Toast.LENGTH_SHORT).show();
