...
public class QuestionUpload extends AppCompatActivity {
    ...
    String facultyvalue,semestervalue,university,sub,q_year,filename;
    RadioGroup uni;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        year=(EditText)findViewById(R.id.author_txt);
        notification=findViewById(R.id.notification);

        Toast.makeText(this, "Question upload page", Toast.LENGTH_SHORT).show();

        selectFile.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(ContextCompat.checkSelfPermission(QuestionUpload.this, Manifest.permission.READ_EXTERNAL_STORAGE )== PackageManager.PERMISSION_GRANTED)
                {
                    selectPdf();
                }
                else
                    ActivityCompat.requestPermissions(QuestionUpload.this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE},9);
            }
        });
        ...
        upload.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                {   sub=subject.getText().toString().trim();
                    q_year=year.getText().toString().trim();
                    ...
                    if (sub.isEmpty() || q_year.isEmpty() || university.isEmpty() || semestervalue.isEmpty()||facultyvalue.isEmpty()) {

                        Toast.makeText(QuestionUpload.this, "Subject name and Author name required", Toast.LENGTH_SHORT).show();
                    ...
                    Toast.makeText(QuestionUpload.this, "Select a File", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void uploadFile(Uri pdfUri) {
        ...
        StorageReference storageReference = storage.getReference(); // returns root path
            ...
            storageReference.child("Questions").child(university).child(facultyvalue).child(semestervalue).child(sub).child(filename).putFile(pdfUri)
...
//                            Toast.makeText(FileUpload.this, url, Toast.LENGTH_SHORT).show();
                            ...
                            QuestionsInfo questionsInfo = new QuestionsInfo(url,q_year,filename1,sub);
                            reference.child("Questions").child(university).child(facultyvalue).child(semestervalue).child(filename1).setValue(questionsInfo).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    progressDialog.dismiss();
                                    if (task.isSuccessful()) {
                                        Toast.makeText(QuestionUpload.this, "File is successfuly uploaded", Toast.LENGTH_SHORT).show();
                                    } else {
                                        Toast.makeText(QuestionUpload.this, "File not uploaded in realtime databse", Toast.LENGTH_SHORT).show();
                                    }
                                }
                ...
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(QuestionUpload.this, "File is not successfuly uploaded in storage.", Toast.LENGTH_SHORT).show();
